...
@include('business-owner.listings.editstyle')


<div class="premium-container" x-data="editListingForm()" x-init="initMap()">
    <header class="page-header">
        ...
        <h1>{{ __('messages.edit_activity_title', ['title' => $listing->title]) }}</h1>
    </header>
    ...
    <form @submit.prevent="submitForm" enctype="multipart/form-data">
        ...
        <div class="form-section">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <select name="listing_type_id" id="listing_type_id" class="form-control" x-model="form.listing_type_id" @change="validateField('listing_type_id')">
                        ...
                        @foreach($listingTypes as $type)
                            <option value="{{ $type->id }}" {{ $listing->listing_type_id == $type->id ? 'selected' : '' }}>
                                {{ $type->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group full-width"
                     x-data="categorySelector(@json($listing->categories->pluck('id')->toArray()))">
                    ...
                    <div class="input-group">
                        <select name="country_id" id="country_id" class="form-control" x-model="form.country_id" @change="validateField('country_id')">
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" {{ $listing->country_id == $country->id ? 'selected' : '' }}>
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
        ...
        <div class="form-section">
            ...
            <div class="form-group full-width spacing-mt-1_5" x-data="featureSelector(@json($listing->features->pluck('id')->toArray()))">
            ...
            <div class="form-group full-width spacing-mt-1_5">
                ...
                <select name="availability_status" class="form-control" style="max-width: 260px;">
                    ...
                    <option value="open" {{ old('availability_status', $listing->availability_status) === 'open' ? 'selected' : '' }}>مفتوح الآن</option>
                    <option value="closed" {{ old('availability_status', $listing->availability_status) === 'closed' ? 'selected' : '' }}>مغلق الآن</option>
                </select>
            </div>

            <div class="form-group full-width spacing-mt-1_5">
                <label class="form-label">إخفاء النشاط</label>
                <label class="checkbox-row" style="display:flex; align-items:center; gap:0.8rem; margin-top:0.25rem;">
                    <input type="checkbox" name="is_hidden" value="1" {{ old('is_hidden', $listing->is_hidden) ? 'checked' : '' }}>
                    <span>إخفاء هذا النشاط من الموقع للجميع</span>
                </label>
                <textarea name="hidden_reason" rows="2" class="form-control" placeholder="سبب الإخفاء (اختياري)" style="margin-top:0.75rem;">{{ old('hidden_reason', $listing->hidden_reason) }}</textarea>
            </div>

            <div class="form-group full-width spacing-mt-1_5">
                <label class="form-label">{{ __('messages.current_images') }}</label>
                <div class="existing-images-grid">
                    @foreach($listing->getMedia('images') as $image)
                        ...
                        @endphp
                        <div class="image-item" x-data="{ checked: false }">
                            <img src="{{ $signedUrl }}" alt="{{ __('messages.activity_image') }}">
                            <label class="remove-check" :class="{ 'selected': checked }">
                                <input type="checkbox" name="remove_images[]" value="{{ $image->id }}" x-model="checked">
                                <span x-text="checked ? '✓' : '✕'"></span>
                            </label>
                        </div>
                    @endforeach
                </div>
                <p class="text-sm text-gray-500 mt-1">{{ __('messages.delete_images_hint') }}</p>
            </div>

            <div class="form-group full-width spacing-mt-1_5">
                <label class="form-label">{{ __('messages.add_new_images') }}</label>
                <div class="modern-dropzone" id="dropzoneArea"
                    ...
                    <input type="file" name="images[]" id="images" multiple accept="image/*" class="file-input-hidden" @change="handleImageUpload($event)">
                </div>
        ...
        <div class="form-group spacing-mt-2_5">
            <button type="submit" class="btn-primary" :disabled="isSubmitting">
                ...
                <span x-text="isSubmitting ? '{{ __('messages.updating') }}' : '{{ __('messages.update_activity') }}'"></span>
            </button>
            <a href="{{ route('business-owner.listings.index') }}" class="btn-secondary" style="margin-right: 1rem;">
                {{ __('messages.cancel') }}
            </a>
        </div>
    </form>
...
<script>
    window.categorySelector = function(existingIds = []) {
        return {
            open: false,
            selectedCats: [],
            init() {
                const categories = @json($categories->flatMap(function($cat) {
                    $items = collect([$cat]);
                    if ($cat->children) {
                        $items = $items->merge($cat->children);
                    }
                    return $items;
                ...
                existingIds.forEach(id => {
                    if (categories[id]) {
                        this.selectedCats.push({ id: id, name: categories[id] });
                    }
                });
            },
    ...
    window.featureSelector = function(existingIds = []) {
        return {
    ...
    window.editListingForm = function() {
        return {
            form: {
                listing_type_id: {{ $listing->listing_type_id }},
                title: '{{ addslashes($listing->title) }}',
                short_description: '{{ addslashes($listing->short_description) }}',
                description: '{{ addslashes($listing->description) }}',
                country_id: {{ $listing->country_id ?? 'null' }},
                city_id: {{ $listing->city_id ?? 'null' }},
                address: '{{ addslashes($listing->address) }}',
                latitude: {{ $listing->latitude ?? 40.4168 }},
                longitude: {{ $listing->longitude ?? -3.7038 }},
            },
            errors: {},
            isSubmitting: false,
            generalError: false,
            ...
            submitForm() {
                ...
                document.querySelectorAll('input[name="remove_images[]"]:checked').forEach(el => {
                    formData.append('remove_images[]', el.value);
                });

                formData.append('_method', 'PUT');

                fetch('{{ route("business-owner.listings.update", $listing) }}', {
                    method: 'POST',
                    headers: {
                    ...
                    files.forEach(file => {
                        if (file.type.match('image.*')) {
                            ...
                            reader.onload = function(e) {
                                ...
                                box.innerHTML = `<img src="${e.target.result}" alt="{{ __('messages.preview') }}">`;
            ...
            initMap() {
                ...
                const lat = parseFloat(this.form.latitude) || 40.4168;
                const lng = parseFloat(this.form.longitude) || -3.7038;
                ...
                const updateCoords = (newLat, newLng) => {
                    ...
                    console.log('✅ تم تحديث الإحداثيات (تعديل):', newLat, newLng);
                };
                ...
                console.log('✅ الخريطة جاهزة (تعديل) مع الإحداثيات:', lat, lng);
            }
        };
    };
</script>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.addEventListener('category-changed', function() {
            const formComponent = document.querySelector('[x-data="editListingForm()"]')?.__x;
