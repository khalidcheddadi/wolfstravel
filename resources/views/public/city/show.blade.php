                        ...
                        <?php foreach($mainCategories as $cat): ?>
                            <li>
                                <a href="{{ route(app()->getLocale() . '.category.show', $cat->slug) }}"
                                   ...
                                   <?php echo ($cat->id == ($city->category_id ?? null))
                                        ? 'bg-primary-50 text-primary-600 font-bold'
                                        : 'text-gray-600 hover:bg-gray-50 hover:text-dark font-medium'; ?>">
                                    <span class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center <?php echo ($cat->id == ($city->category_id ?? null)) ? 'bg-primary-100' : 'bg-gray-100 group-hover:bg-white group-hover:shadow-sm transition-all'; ?>">
                                            <i class="fa-solid fa-hashtag text-xs"></i>
                                        </div>
                                        {{ $cat->name }}
                                    </span>
                                    <span class="text-xs py-1 px-2.5 rounded-full bg-white border border-gray-100 shadow-sm <?php echo ($cat->id == ($city->category_id ?? null)) ? 'text-primary-600' : 'text-gray-400'; ?>">
                                        {{ $cat->listings_count }}
                                    </span>
                                </a>
                            </li>
            ...
            <div class="w-full lg:w-3/4">
                            ...
                            <div class="group bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] hover:-translate-y-1 transition-all duration-400"
                                ...
                                <div class="relative w-full aspect-[4/3] overflow-hidden bg-gray-100">
                                    <a href="{{ route(app()->getLocale() . '.listing.show', $listing->slug) }}" class="block w-full h-full">
                                        <?php $imageUrl = $listing->getSignedImageUrl('medium', 60); ?>
                                        <?php if($imageUrl): ?>
                                            <img src="{{ $imageUrl }}"
                                                 alt="{{ $listing->title }}"
                                    ...
                                    <div class="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-dark text-xs font-bold px-3 py-1.5 rounded-full shadow-sm flex items-center gap-1.5">
                                        <i class="fas <?php echo $listing->type?->icon ?? 'fa-tag'; ?> text-primary-600"></i>
                                        <span><?php echo $listing->type?->name ?? __('messages.listing_default_activity'); ?></span>
                                    </div>
                                ...
                                <div class="p-5 lg:p-6">
                                    ...
                                    <div class="flex flex-col gap-2.5 mb-5">
                                        <div class="flex items-center gap-2.5 text-sm text-gray-500">
                                            ...
                                            <span class="truncate">{{ $listing->city?->name }}</span>
                                        </div>
                                        <div class="flex items-center gap-2.5 text-sm text-gray-500">
                                            <div class="w-6 h-6 rounded-full bg-gray-50 flex items-center justify-center text-gray-400">
                                                <i class="fas fa-phone text-xs"></i>
                                            </div>
                                            <span dir="ltr" class="font-medium tracking-wide">{{ $listing->business?->phone ?? '+212 00 00 00 00' }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                        <div class="flex items-center gap-3">
                                            ...
                                            {{-- Favorite Button --}}
                                            <?php if(auth()->check()): ?>
                                                <?php $isFavorited = auth()->user()->favorites()->where('listing_id', $listing->id)->exists(); ?>
                                                <form action="{{ route('favorite.toggle', $listing) }}" method="POST">
                                                    @csrf
                                                    <button type="submit"
                                                            class="w-9 h-9 rounded-full flex items-center justify-center transition-all
                                                            <?php echo $isFavorited ? 'bg-red-50 text-red-500' : 'text-gray-400 hover:bg-red-50 hover:text-red-500'; ?>"
                                                            title="<?php echo $isFavorited ? __('messages.listing_remove_favorite') : __('messages.listing_add_favorite'); ?>">
                                                        <i class="<?php echo $isFavorited ? 'fas scale-110' : 'far'; ?> fa-heart transition-transform"></i>
                                                    </button>
                                                </form>
                                            <?php endif; ?>
                                        </div>
                ...
                <?php else: ?>
                    ...
                    <div class="bg-white rounded-3xl border border-gray-100 p-12 text-center shadow-sm flex flex-col items-center" data-aos="zoom-in">
                        ...
                        <p class="text-gray-500 mb-8 max-w-md">لم نتمكن من العثور على أي أنشطة في هذه المدينة أو هذا التصنيف في الوقت الحالي. تحقق مرة أخرى لاحقاً!</p>
